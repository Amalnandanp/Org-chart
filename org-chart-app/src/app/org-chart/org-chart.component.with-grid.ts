    ...
    if (shouldGrid && (node.children || node._gridRemainder)) {
      ...
      if (allChildren.length > 0) {
        ...
        allChildren.forEach((child: any, index: number) => {
          ...
          // Set grid position
          const targetX = startX + (col * itemWidth);
          const targetY = startY + (row * itemHeight);

          child.x = targetX;
          child.y = targetY;

          // FIX: Re-layout subtree for grid items
          // Since we moved this node (or it was hidden from D3), we must layout its descendants
          // relative to its new position.
          if (child.children && child.children.length > 0) {
            // Run tree layout on the subtree
            this.tree(child);

            // Calculate shift needed to put child back at target position
            // this.tree(child) will likely place child at (0,0) or similar relative coords
            const dx = targetX - child.x;
            const dy = targetY - child.y;

            // Apply shift to child and all descendants
            child.descendants().forEach((d: any) => {
              d.x += dx;
              d.y += dy;
            });
          }
